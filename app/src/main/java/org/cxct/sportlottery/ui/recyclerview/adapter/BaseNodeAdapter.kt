package org.cxct.sportlottery.ui.recyclerview.adapter
...
abstract class BaseNodeAdapter(nodeList: MutableList<out BaseNode>? = null)
...
//        notifyItemRangeChanged(index + getHeaderLayoutCount(), max(removeCount, newFlatData.size)
     ...
     * @param newData 添加的数据集合
     */
    fun nodeAddData(parentNode: BaseNode, newData: Collection<BaseNode>) {
        parentNode.childNode?.let {
            it.addAll(newData)
