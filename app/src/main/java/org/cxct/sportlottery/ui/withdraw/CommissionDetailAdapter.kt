        ...
        try {
            ...
            viewHolder.run {
                ...
                tvRequiredValidBetsMoney.apply {
                    ...
                    setTextColor(ContextCompat.getColor(context, if(data.validCheckAmount ?: 0.0 < 0.0) R.color.colorRed else R.color.color_e5e5e5_333333))
                }

                tvSuccessedBetsMoney.apply {
                    setMoneyFormat(data.finishValidAmount?.toLong()?.toDouble() ?: 0.0)
                    setTextColor(ContextCompat.getColor(context, if(data.finishValidAmount ?: 0.0 < 0.0) R.color.colorRed else R.color.color_e5e5e5_333333))
                }

                tvDeductMoney.apply {
                    setMoneyFormat(zero.minus(data.deductMoney ?: 0))
                    setTextColor(ContextCompat.getColor(context, if(zero.minus(data.deductMoney ?: 0) < 0.0) R.color.colorRed else R.color.color_e5e5e5_333333))
                }
