...
class RvBYAdapter : RecyclerView.Adapter<RvBYAdapter.ItemViewHolder>() {
    ...
    inner class ItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int, data: ThirdDictValues?) {
            ...
            val bgUrl = GameConfigManager.getThirdGameHallIcon(data?.gameCategory, bgCode)
            ...
            val logoUrl = GameConfigManager.getThirdGameHallIcon(data?.gameCategory, data?.firmCode)
            Glide.with(itemView.context)
                .load(logoUrl)
                .apply(mRequestOptions)
                .thumbnail(0.5f)
