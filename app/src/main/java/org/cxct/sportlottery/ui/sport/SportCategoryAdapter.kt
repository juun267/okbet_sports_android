...
class SportCategoryAdapter : RecyclerView.Adapter<SportCategoryAdapter.ViewHolderPlayCategory>() {
    ...
    var sportCategoryListener: SportCategoryListener? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderPlayCategory {
        return ViewHolderPlayCategory(LayoutInflater.from(parent.context)
    ...
    override fun onBindViewHolder(holder: ViewHolderPlayCategory, position: Int) {
        holder.bind(data[position], sportCategoryListener)
    }
    ...
    inner class ViewHolderPlayCategory constructor(itemView: View) :
        ...
        fun bind(item: Play, sportCategoryListener: SportCategoryListener?) {
            var initSpinnerAdapter = true
            var initialSetItem: Boolean
            val playSpinnerAdapter by lazy {
                PlaySpinnerAdapter(item.playCateList?.toMutableList() ?: mutableListOf())
            }
                            ...
                            item.playCateList?.let { playList ->
                                ...
                                if (!initSpinnerAdapter && !initialSetItem) {
                                    playList.getOrNull(position)?.let { playCate ->
                                        sportCategoryListener?.onSelectPlayCateListener(item,
                                            playCate,
                                            playList.any { it.isSelected })
                ...
                if (item.selectionType == SelectionType.SELECTABLE.code) {
                    when {
                        ...
                        !item.isSelected && item.isLocked == false -> {
                            sportCategoryListener?.onClickSetItemListener(item)
                        }
                    }
                } else {
                    sportCategoryListener?.onClickNotSelectableListener(item)
                }
            }
        }
...
class SportCategoryListener(
    ...
    fun onSelectPlayCateListener(item: Play, selectItem: PlayCate, hasItemSelect: Boolean) =
        onSelectPlayCateListener.invoke(item, selectItem, hasItemSelect)
}