...
class OKLiveOkGamesView(context: Context, attrs: AttributeSet) : RelativeLayout(context, attrs) {
    ...
    private fun initView() {
        LayoutInflater.from(context).inflate(R.layout.view_home_oklive_okgame, this, true)
    ...
    fun <T : OKLiveViewModel> setOkGamesData(fragment: BaseBottomNavigationFragment<T>?) {
        if (fragment == null) {
            return
        }
        //请求games数据
     ...
     * 更换页码
     */
    private fun <T : OKLiveViewModel> changePageData(
        isBackPage: Boolean, fragment: BaseBottomNavigationFragment<T>
    ) {
        ...
        if (totalGameMap.containsKey(viewModel.pageIndexLiveData.value)) {
            ...
            //请求该页数据
            fragment.loading()
            viewModel.getOkLiveOKGamesList()
        }
    }
