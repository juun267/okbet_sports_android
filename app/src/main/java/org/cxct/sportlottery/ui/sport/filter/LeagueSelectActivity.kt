...
class LeagueSelectActivity :
    ...
    private fun setIndexbar(indexArr: Array<CharSequence>, indexOfChar: (CharSequence) -> Int) {
        //自定义索引数组，默认是26个大写字母
        binding.indexBar.textArray = indexArr
        //添加相关监听
        binding.indexBar.setOnIndexLetterChangedListener(object : IndexBar.OnIndexLetterChangedListener {
            override fun onTouched(touched: Boolean) {
                //TODO 手指按下和抬起会回调这里
                binding.ivUnion.isVisible = touched
            }
            ...
            override fun onLetterChanged(indexChar: CharSequence, index: Int, y: Float) {
                ...
                binding.ivUnion.y = y + binding.cvIndex.top
                binding.ivUnion.text = indexChar
            }
        })
    }
