...
object SvgUtil {
    ...
    private val svgFormat = "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='%d' height='%d' viewBox='0 0 %d %d' >%5\$s</svg>"
    ...
    fun ImageView.setSvgIcon(svgData: String, @DrawableRes errorIcon: Int) {
        ...
        val svgIcon = svgFormatDataDrawable(svgData)
        if (svgIcon == null) {
    ...
    private fun svgFormatDataDrawable(svgFormatData: String): PictureDrawable? {
        val data = String.format(svgFormat, 48, 48, 24, 24, svgFormatData)
        val svgFile = runCatching { SVG.getFromString(data) }.getOrNull() ?: return null
        return PictureDrawable(svgFile.renderToPicture())
    }

    private fun getSvgDrawable(svgData: String): PictureDrawable?  {
