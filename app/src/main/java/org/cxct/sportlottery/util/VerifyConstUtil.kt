...
object VerifyConstUtil {
    ...
    fun verifyValidCode(code: CharSequence): Boolean {
        return Pattern.matches("[$NUMBER$ENGLISH_WORD]{1,4}", code)
    }
    ...
    fun verifyWeChat(text: CharSequence): Boolean {
        return Pattern.compile("^[a-zA-Z][-_a-zA-Z0-9]{5,20}$").matcher(text).matches()
    }

    //Telegram //可以數字、英文或底線, 5~32個char
    fun verifyTelegram(text: CharSequence): Boolean {
        return Pattern.compile("^[-_a-zA-Z0-9]{5,32}$").matcher(text).matches()
    }
