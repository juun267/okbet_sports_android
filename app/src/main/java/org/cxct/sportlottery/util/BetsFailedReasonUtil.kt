...
object BetsFailedReasonUtil {
    ...
    fun getFailedReasonByCode(code: String?): String {
        val instance = MultiLanguagesApplication.getInstance()
        var reason = ""
        if (instance == null) return ""
        when (code) {
            ODDS_CHANGED -> {
                reason = instance.getString(R.string.str_odds_changed)
            } //赔率变化
            COST_TOO_LONG_TIME_TO_CONFIRM -> {
                reason = instance.getString(R.string.str_bet_slip_not_confirm_for_a_long_time)
            } //注单长时间未确认
            BET_MONEY_EXCEED_FOR_SINGLE_BET,
            BET_MONEY_EXCEED_FOR_MATCH_BET,
            BET_MONEY_EXCEED_FOR_MATCH_PLAY,
            BET_MONEY_EXCEED_FOR_USER -> {
                reason = instance.getString(R.string.str_exceeding_limit_for_the_event_or_market)
            } // 用户超出单日投注上限
            BET_IN_RISK_TIME ,
            BET_MATCH_STATUS_CHANGED ,
            MATCH_STATUS_CHANGED -> {
                reason = instance.getString(R.string.str_market_closed_please_try_later)
            } //赛事状态异动
            MATCH_STATUS_CLOSE -> {
                reason = instance.getString(R.string.str_match_closed)
            }//赛事不存在
            ODDS_SUSPENDED -> {
                reason = instance.getString(R.string.str_match_be_suspended)
            }//赛事暂停
            ODDS_TYPE_ERROR -> {
                reason = instance.getString(R.string.str_match_be_suspended)
            }//盘口错误
            MONEY_ERROR -> {
                reason = instance.getString(R.string.str_amount_wrong)
            }//金额不正确
            STAKE_GT_MAX_BET_LIMIT -> {
                reason = instance.getString(R.string.str_exceeding_limit_for_the_event_or_market)
            }//下注金额超过最大投注额
            STAKE_LT_MIN_BET_LIMIT -> {
                reason = instance.getString(R.string.str_less_than_min_bet)
            }//下注金额低于最小投注额
            ...
            PARLAY_HAVE_OTHER_SPORT -> {
                reason = instance.getString(R.string.str_only_one_type_of_ball_can_be_used_parlay)
            }//串关只能同一种球类
            PARLAY_NUM_IS_OVER -> {
                reason = instance.getString(R.string.str_parlay_exceed_the_ties_limit)
            }//串关数量超出限制
            STAKE_GT_SELF_LIMIT -> {
                reason = instance.getString(R.string.str_self_exclusion)
            }//自我禁制
            SOURCE_ERROR -> {
                reason = instance.getString(R.string.str_match_be_suspended)
            }//赛事数据源错误
            PARLAY_ODD_NUM_ERROR -> {
                reason = instance.getString(R.string.str_match_be_suspended_or_closed_please_check)
            }// 串关可以下注的赔率数量与玩家下注的赔率数量不一致
            OUTRIGHT_PARLAY_ERROR -> {
                reason = instance.getString(R.string.str_parlay_is_not_allowed_in_championship_matches)
            }// 冠军赛事禁止串关
            PARLAY_TYPE_ERROR -> {
                reason = instance.getString(R.string.str_parlay_type_wrong)
            }// 串关类型错误
        }
        return reason
    }
